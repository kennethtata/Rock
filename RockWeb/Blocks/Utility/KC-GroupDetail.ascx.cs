...
namespace RockWeb.Blocks.Utility
{
    ...
    public partial class KC_GroupDetail : Rock.Web.UI.RockBlock
    {
        ...
        {
            int groupId = PageParameter("GroupId").AsIntegerOrNull() ?? 0;
            ...
            if (group == null)
            {
                ltDetails.Text = "<div class='alert alert-warning'>Group not found.</div>";
                return;
            }
            ...
            ltDetails.Text = $@"
                ...
                <p><strong>Date Created:</strong> {group.CreatedDateTime?.ToShortDateString() ?? "(unknown)"}</p>
                <p><strong>Date Modified:</strong> {group.ModifiedDateTime?.ToShortDateString() ?? "(unknown)"}</p>
